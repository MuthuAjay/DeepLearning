...
class Datasets():
    ...
    def circle_dataset(self, n_samples: int = 1000,
        ...
        X, y = torch.from_numpy(X).type(torch.float).to(Utils.set_device()) , torch.from_numpy(y).type(torch.float).to(Utils.set_device())
    ...
    def blob_dataset(self,
        ...
        X,y = torch.from_numpy(X).type(torch.float).to(Utils.set_device()), torch.from_numpy(y).type(torch.LongTensor).to(Utils.set_device())
        X_train, X_test, y_train, y_test = self.create_train_test_split(X,y)
